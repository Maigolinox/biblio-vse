...
\begin{abstract}
Compliance auditing of software engineering governance standards such as ISO/IEC 29110 in corporate repositories remains a manual, costly, and error-prone process. While Large Language Models (LLMs) offer the potential to automate such auditing, two fundamental barriers impede their empirical evaluation: (1) strict industrial privacy policies prevent the publication of corporate datasets, and (2) naive, uncalibrated LLMs produce unreliable classifications due to hallucinations and lack of normative grounding. This paper proposes a methodological framework based on distributional calibration to generate privacy-preserving synthetic datasets, coupled with targeted model enhancements to reduce unsupported or inconsistent classifications. We formalize a set of organizational meta-rules derived from ISO/IEC 29110 and synthesize a controlled environment (Biblio-VSE) that is \emph{distributionally calibrated under selected metrics} relative to a private industrial repository: all three distributional mean-threshold criteria pass with the $N=32$ corpus: V(G) mean ($|\Delta\mu|=0.096\leq\varepsilon=0.5$), AST depth mean ($|\Delta\mu|=0.61\leq\varepsilon_{\text{AST}}=2.0$), and Fog Index mean ($|\Delta\mu_F|=0.22\leq\varepsilon_F=1.0$); Lexical Density fails its bound and equivalence cannot be formally declared at these sample sizes. The benchmark approximates rather than fully replicates the reference corpus. Four open-weights foundation models (Gemma-2, Mistral, Qwen2.5, Phi-3.5) are evaluated across eight progressive configurations encompassing Zero-Shot prompting, Retrieval-Augmented Generation (RAG), and Activation Steering ($\alpha=0.8$) on an extended dataset of $N=32$ artifacts (17 compliant, 15 violations) that includes 12 adversarial artifacts. Statistical analysis uses exact cluster-level paired permutation tests over the nine meta-rule groups (primary) and exact McNemar tests (secondary) with Holm--Bonferroni correction, grouped (cluster) bootstrap confidence intervals, an explicit asymmetric cost function, and complementary metrics (MCC, balanced accuracy, per-rule macro-F1). Retrieval-Augmented Generation over the ISO/IEC 29110 normative corpus (14 semantic chunks from \texttt{Metareglas extraidas.docx}) is the only single intervention that improves the point estimate for all four models---Gemma-2 achieves F1=0.875 under Zero-Shot+RAG (Exp~5)---although no individual gain survives Holm--Bonferroni correction. Activation steering is beneficial only in combination with structured prompting: Zero-Shot+Steering (Exp~6) improves or matches Zero-Shot for all four architectures (steering leaders: Gemma-2 Exp~6 F1=0.857, Phi-3.5 Exp~6 F1=0.842), whereas steering in isolation (Exp~4) \emph{decreases} F1 for Mistral (0.686 to 0.667) and Qwen2.5 (0.727 to 0.703). Pipeline gains are architecture-specific and Zero-Shot alone is destructive for Phi-3.5-mini. \textbf{All F1 figures for steering-enhanced configurations should be treated as upper-bound estimates}; a leave-one-out $\alpha$-selection procedure estimates mean absolute optimism at approximately 0.05 (range $-0.20$ to $+0.09$) within the benchmark, but this does not constitute external validation. On the 12 adversarial artifacts, LLM pipelines exceed both the regex baseline (F1=0.647 on $N=32$) and TF-IDF lexical classifiers (F1=0.63--0.70 under leave-one-group-out cross-validation), suggesting contextual discrimination beyond the evaluated surface-cue baselines---though not establishing human-like semantic reasoning. Given the evaluation set of $N=32$ artifacts, all results are exploratory and hypothesis-generating rather than confirmatory: no improvement is statistically confirmed. The study demonstrates the technical feasibility of evaluating LLM auditing pipelines on a researcher-constructed synthetic benchmark without disclosing industrial data; independent expert labeling, clustered confirmatory testing at larger scale, and evaluation on additional repositories are required before practical auditing claims can be made.
\end{abstract}
...
This article addresses these gaps through a mechanistic LLM control framework for ISO/IEC~29110 compliance auditing. The central hypothesis under evaluation is that activation steering---directly modifying internal model representations toward a normative compliance direction---improves compliance classification across diverse LLM architectures more reliably than prompt engineering or retrieval augmentation alone. The results only partially support this hypothesis: steering improves or matches every architecture when combined with structured prompting, but degrades two of the four models when applied in isolation, whereas retrieval augmentation is the only single intervention with a uniformly positive direction of effect. To evaluate this under industrial privacy constraints, the framework constructs a synthetic benchmark (Biblio-VSE) that is \emph{distributionally calibrated under selected metrics} against a certified industrial repository, replacing proprietary artifacts with non-sensitive equivalents (the term \emph{partial structural isomorphism}, used in earlier drafts, overstates what the calibration evidence establishes; see §\ref{sec:isomorphism}). The calibration methodology is not a peripheral detail---it is the necessary foundation that makes a reproducible mechanistic evaluation possible under real-world privacy requirements. Figure~\ref{fig:conceptual_framework} summarizes the relationship among industrial compliance evidence, privacy-preserving structural isomorphism, and LLM-based mechanistic auditing.
...
\caption{Conceptual framework linking ISO/IEC 29110 industrial repositories, privacy-preserving distributional calibration, synthetic benchmark construction, and LLM-based compliance auditing.}
\label{fig:conceptual_framework}
\end{figure}

The \textbf{primary contribution} is the first systematic evaluation of activation steering for domain-specific normative compliance classification: we show that modifying internal LLM representations toward a compliance direction improves or matches structured prompting for all four evaluated architectures when the two are combined (Exp~6), while steering in isolation degrades two of the four---a nuanced result whose $\alpha$-selection optimism is quantified within the benchmark by a leave-one-out procedure \cite{zou2023representation,turner2023activation,li2023iti}. Three supporting contributions enable and contextualize this mechanistic study. First, a \emph{privacy-preserving benchmark construction strategy} based on distributional calibration under selected metrics approximates measurable properties of an ISO/IEC~29110-certified industrial repository without disclosing proprietary content---the necessary foundation for reproducible evaluation under industrial privacy constraints; the benchmark is also the first in this domain to include adversarial artifacts that defeat regex-based and TF-IDF lexical baselines (Regex F1 drops from 1.0 to 0.647; TF-IDF reaches only 0.63--0.70), indicating that the evaluation exercises contextual discrimination beyond the evaluated surface-cue baselines \cite{bauer2024syntheticdata,dwork2006differentialprivacy,mcmahan2017communication}. Second, the article formalizes ISO/IEC~29110 compliance auditing as a binary classification problem, derives nine meta-rules with explicit detection predicates, and identifies the two semantically hard rules (GOVERNANCE, TESTING) where pattern-based approaches fail and LLM-based auditing provides its differential value \cite{iso29110_2024,terronmacias,11276002,castellanos2022processcompliance}. Third, the article provides the \emph{first controlled comparison} of eight progressive pipeline configurations---from unstructured baseline to full Zero-Shot + RAG + Steering synergy---for ISO/IEC~29110 governance auditing, including a per-meta-rule analysis of which hard cases benefit from each intervention \cite{fuchss2025lissa,laporte2016vse,munoz2020implementations,101145}.

The remainder of this article is organized as follows. Section~\ref{sec:slr_methodology} describes the systematic literature review protocol used to identify and synthesize prior work. Section~\ref{sec:related_work} presents the related work derived from the review, organized around LLMs for software engineering, privacy-preserving benchmarking, activation steering, and automated compliance auditing. Section~\ref{sec:methodology} introduces the proposed distributional calibration and activation-steering framework. Section~\ref{sec:setup} describes the experimental design, datasets, models, and baselines. Section~\ref{sec:results} reports the empirical results. Section~\ref{sec:discussion} discusses implications, limitations, and threats to validity. Section~\ref{sec:conclusion} concludes the article and outlines future research directions.
...
] coordinates {};  %% D_synthetic: n=20, mean=1.94, var=6.45 (isomorphism_metrics.py)
\end{axis}
\begin{axis}[
    boxplot/draw direction=y,
...
\caption{Isomorphism comparison: $D_{private}$ (blue) vs.\ $D_{synthetic}$ (green), $N=32$ dataset. (a) McCabe's $V(G)$ ($n_{\text{priv}}=37$, $n_{\text{synth}}=20$): mean $|\Delta\mu|=0.096\leq\varepsilon=0.5$ \checkmark; variance: Brown--Forsythe Levene test $W=0.000$, $p=0.997$ (the distributions cannot be distinguished at these sample sizes; this is not positive evidence of equivalence, see §\ref{sec:threats}). (b) Gunning Fog Index ($n_{\text{priv}}=87$, $n_{\text{synth}}=12$): $|\Delta\mu_F|=0.22\leq\varepsilon_F=1.0$ \checkmark; $D_{private}$ outlier at 89.44 clipped. All mean-threshold criteria shown here pass; Lexical Density (not shown) fails its bound.}
...
\node[box, right=of private, fill=green!10] (mapping) {Mapping Function $f$ \\ Distributional Calibration};
...
\caption{Privacy-preserving dataset generation via distributional calibration under selected metrics. Four calibrated mean thresholds constrain the synthetic benchmark toward distributional similarity: $\mu(V(G))$, AST depth, and Fog Index pass their bounds; Lexical Density fails ($|\Delta\mu_{L_d}|=14.0>\varepsilon_{L_d}=8.0$). V(G) variance is assessed with the Brown--Forsythe Levene test (no positive equivalence claim).}
...
Two caveats bound what this analysis establishes. First, both import graphs are dominated by edges to the Django framework and the standard library, and the internal coupling ratio is zero in both corpora: the matched quantities are therefore properties of framework usage style, not of project-internal architecture. Second, matching marginal density and out-degree statistics does not establish comparable \emph{governance topology}---the web of approvals, reviews, and traceability relations that compliance auditing actually concerns. The import-graph analysis, combined with the scalar metrics, supports only the claim that the benchmark is \emph{distributionally calibrated under the selected metrics}; it does not validate structural equivalence in any stronger sense.
...
The F1-Score was utilized as the primary evaluation metric, balancing Precision and Recall over the $N=32$ artifacts. Because F1 alone obscures asymmetric auditing costs---a false declaration of compliance (FP) is typically more damaging in an audit context than a false alarm (FN)---each configuration is additionally reported with Matthews Correlation Coefficient (MCC), balanced accuracy, and per-meta-rule macro-F1 (the mean of the nine within-rule binary F1 scores) in Table~\ref{tab:full_results}.

\textbf{Grouped bootstrap confidence intervals.} The 32 artifacts are not independent observations: they are paired compliant/violating mutations, and adversarial extensions, generated per meta-rule from a single synthetic Django project. Artifact-level i.i.d.\ resampling would therefore assume more independence than the design provides. To respect this dependence structure, we employ a grouped (cluster) bootstrap with $k=10{,}000$ iterations: the nine meta-rule groups are resampled with replacement as intact clusters, and the F1-Score is recalculated on each resample. Traditional $k$-fold cross-validation was intentionally avoided due to the limited dataset size, which would produce high-variance partitioning effects. The resulting empirical distribution yields 95\% confidence intervals (CI):
...
where $Q_p$ denotes the empirical percentile function and $\hat{F}_1^{(b)}$ denotes the F1-Score obtained during cluster-bootstrap iteration $b \in \{1,\dots,10{,}000\}$. With only nine clusters, these intervals are themselves approximate and should be read as plausibility ranges rather than exact coverage statements.

\textbf{Paired significance testing.} The \emph{primary} comparison between each configuration and its model baseline (Experiment~1) is an exact \emph{cluster-level paired permutation test} that respects the dependence structure: the per-cluster correctness difference (configuration minus baseline) is computed for each of the nine meta-rule clusters, and the cluster signs are flipped exhaustively ($2^9=512$ exact enumeration); the two-sided $p$-value is the fraction of sign patterns whose summed statistic is at least as extreme as the observed one. As a \emph{secondary} test we also report the exact McNemar test on paired correctness outcomes (discordant pairs $b$ = baseline-only correct, $c$ = candidate-only correct, tested against $\mathrm{Binomial}(b+c, 0.5)$); McNemar treats the 32 artifact pairs as independent and therefore overstates the effective sample size under this design. An earlier version of this analysis used the Wilcoxon signed-rank test on paired binary predictions; that test detects shifts in the output-label distribution, not differences in classification accuracy, and was replaced. All per-artifact predictions and the analysis code are released as a canonical machine-readable matrix (\texttt{experimentos/predictions\_n32.json}, \texttt{.csv}) and script (\texttt{experimentos/statistical\_resampling.py}).

\textbf{Asymmetric cost function.} Because falsely declaring compliance (FP) is typically costlier in auditing than a false alarm (FN), each configuration is additionally evaluated under an explicit expected-cost function
\begin{equation}
C = \frac{c_{FP}\,\mathrm{FP} + c_{FN}\,\mathrm{FN}}{N}, \qquad c_{FN}=1,\; c_{FP}\in\{1,2,5,10\},
\label{eq:cost}
\end{equation}
and the cost-minimizing configuration per model is reported for each cost ratio (Table~\ref{tab:cost_sensitivity}).

\textbf{Statistical power.} With only nine clusters, the exact permutation test has a minimum attainable two-sided $p$-value of $2/512\approx0.004$, and in practice ties and small per-cluster differences keep attainable significance far coarser; confirmatory inference is weak at this design size regardless of test choice. The secondary McNemar test depends on the number of discordant pairs (observed range: 2--13); a configuration must correct at least six baseline errors without introducing any new error before reaching $p<0.05$ \emph{uncorrected}. Small-to-medium accuracy differences are thus not reliably detectable, and the study should be interpreted as a proof-of-concept exploration rather than a definitive power trial.

\textbf{Multiple comparison correction.} Each test family comprises 28 comparisons (7 non-baseline configurations $\times$ 4 models). The family-wise error rate without correction is approximately $1-(1-0.05)^{28} \approx 0.76$. Holm--Bonferroni step-down correction was applied within each family; no comparison in either family survives correction (every Holm-adjusted $p$-value equals 1.000), and no raw $p$-value in either family reaches 0.05. Moreover, this correction covers only the configuration-vs-baseline tests; it does not account for the additional selection dimensions explored during the study (steering layer, $\alpha$, anchor language, and the choice of best model/configuration among 32 cells), so any ``best cell'' highlighted in this paper is additionally subject to winner's-curse selection bias and should be treated as descriptive only.
...
\node[box, right=of private, fill=green!10] (iso) {Distributional Calibration Mapping ($f$)};
...
This section presents the empirical evaluation of all experimental configurations. Interpretation and implications are deferred to Section~\ref{sec:discussion}. A global overview of performance across all 32 model-configuration cells on the $N=32$ artifact set is provided in the heatmap in Fig.~\ref{fig:heatmap}. Results labeled ``LOO-F1'' for steering configurations (Exp~4, 6, 7, 8) are leave-one-out estimates of $\alpha$-selection optimism, internal to the benchmark, from the procedure described in §\ref{sec:pipeline}.
...
Experiment~4 applied activation steering ($\alpha=0.8$) without contextual or structural prompting, using Spanish-language anchor prompts as the methodologically correct design. Gemma-2 achieved F1=0.8205, Phi-3.5 reached F1=0.7556, Qwen2.5 F1=0.7027, and Mistral F1=0.6667. Steering shifts configurations toward higher recall, as visible in Fig.~\ref{fig:precision_recall}. Note that $\alpha=0.8$ was selected by pilot experimentation on the evaluation set; the LOO procedure quantifies the resulting selection optimism (LOO-F1 in Table~\ref{tab:full_results}; mean $|$optimism$| \approx 0.05$, see §\ref{sec:threats}). Relative to baseline, steering-only is a net loss on cost-aware metrics for three models: it lowers MCC for Mistral and Qwen2.5 and lowers balanced accuracy for Phi-3.5 (Table~\ref{tab:full_results}).
...
\subsection{Non-LLM Baselines: Regex and Lexical Classifiers}

On the original 20 artifacts, the deterministic Regex baseline achieved perfect classification (F1=1.0000, $CI_{95\%}$: [1.00, 1.00]), confirming label validity for the core dataset. On the extended $N=32$ dataset including adversarial artifacts, the Regex baseline drops to F1=0.6471 (grouped 95\,\% CI [0.59, 0.71]), which is comparable to---and indistinguishable from---the majority-class floor of F1=0.6939. The adversarial artifacts defeat this particular pattern matcher: the Type-B negatives contain compliance keywords in non-compliant contexts (causing false positives), and the Type-C positives comply without the obvious keyword signatures (causing false negatives).

Because outperforming a single handcrafted regex does not by itself rule out that LLMs exploit \emph{other} surface cues (alternative vocabulary, formatting, file structure), three shallow lexical classifiers were added (\texttt{experimentos/lexical\_baselines.py}): TF-IDF over word 1--2 grams with logistic regression and with a linear SVM, and TF-IDF over character 3--5 grams with logistic regression, all evaluated with leave-one-meta-rule-group-out cross-validation so that no lexical material from the tested artifact family is seen during training. These classifiers reach F1 = 0.667, 0.703, and 0.634 respectively---at or below the majority-class floor and comparable to the regex. LLM-based pipelines (F1 = 0.75--0.875) substantially exceed all evaluated surface-cue baselines, which \emph{suggests contextual discrimination beyond the evaluated regex and lexical baselines}; it does not prove human-like semantic reasoning, since unevaluated cues (e.g., formatting or structural regularities) cannot be excluded without paraphrase and cue-removal perturbation tests, which remain future work. The perfect-Regex result on the original 20 artifacts should be retained as a \emph{label-validity check} for those artifacts only; it does not imply that LLM-based auditing is unnecessary.

\subsection{Statistical Robustness}

Table~\ref{tab:full_results} presents the complete experimental results with grouped bootstrap confidence intervals, MCC, balanced accuracy, per-rule macro-F1, and raw $p$-values from both test families (cluster-level paired permutation as primary, exact McNemar as secondary; Holm--Bonferroni applied within each $m=28$ family) on the $N=32$ artifact set. The majority-class floor is F1=0.6939; most configurations exceed it. No comparison in either family survives Holm correction at $\alpha=0.05$, and no raw $p$-value reaches 0.05: the smallest permutation $p$ is 0.1094 (Mistral Exp~6) and the smallest McNemar $p$ is 0.0625 (Gemma-2 Exp~6/8). The strongest statement these data support is therefore: \emph{RAG and selected steering combinations show promising directional effects on this benchmark, but no improvement is statistically confirmed.} In particular, the point-estimate maximum (Gemma-2 Exp~5, F1=0.875) should not be interpreted as evidence that this model-configuration cell is superior. The study is exploratory and hypothesis-generating; the observed trends motivate further evaluation on larger datasets. The MCC and balanced-accuracy columns reveal patterns that F1 masks: steering-only (Exp~4) \emph{lowers} MCC relative to baseline for Mistral (0.177 vs.\ 0.308) and Qwen2.5 (0.307 vs.\ 0.438), and lowers balanced accuracy for Phi-3.5 (0.633 vs.\ 0.686) despite a higher F1---the recall gains come at the cost of false-compliance declarations, the costlier error type in auditing. For steering configurations, Table~\ref{tab:full_results} additionally reports LOO-F1 (leave-one-out estimates from the $\alpha$-selection procedure described in §\ref{sec:pipeline}); mean absolute optimism is approximately 0.05 (range $-0.20$ to $+0.09$), indicating that the in-sample F1 values are not substantially inflated on average; Qwen2.5 Exp~7 is an exception (LOO-F1=0.556).

As visualized in Fig.~\ref{fig:forest_plot}, the grouped confidence intervals remain wide and overlap substantially across configurations; no configuration is distinguishable from the others with statistical confidence at this sample size.

\subsection{Cost-Sensitive Evaluation}

Table~\ref{tab:cost_sensitivity} reports, for each model, the configuration that minimizes the expected cost of Eq.~\ref{eq:cost} as the false-compliance penalty $c_{FP}$ grows. The ranking shifts materially: as $c_{FP}$ increases from 1 to 10, high-precision RAG-anchored configurations displace the F1 leaders for three of the four models (Gemma-2 and Phi-3.5 move to RAG-only; Qwen2.5 to RAG+Steering), because RAG's conservative classification pattern (few FP) is exactly what an FP-averse auditor needs. Mistral is the exception: ZS+Steering remains cost-minimal at all evaluated ratios. This confirms that F1-based rankings do not transfer to cost-asymmetric auditing deployments and that configuration selection should be driven by the organization's cost structure.

\begin{table}[htbp]
\centering
\caption{Cost-minimizing configuration per model under the expected-cost function of Eq.~\ref{eq:cost} ($c_{FN}=1$; normalized cost in parentheses). As the false-compliance penalty grows, high-precision RAG configurations dominate for three of four models.}
\label{tab:cost_sensitivity}
\renewcommand{\arraystretch}{1.2}
\resizebox{\columnwidth}{!}{%
\begin{tabular}{lllll}
\toprule
\textbf{Model} & $c_{FP}{=}1$ & $c_{FP}{=}2$ & $c_{FP}{=}5$ & $c_{FP}{=}10$ \\
\midrule
Gemma-2  & ZS+RAG (0.125) & ZS+RAG (0.156) & RAG (0.188) & RAG (0.188) \\
Mistral  & ZS+Steer (0.156) & ZS+Steer (0.219) & ZS+Steer (0.406) & ZS+Steer (0.719) \\
Qwen2.5  & ZS (0.188) & ZS+RAG (0.250) & RAG+Steer (0.344) & RAG+Steer (0.500) \\
Phi-3.5  & RAG (0.188) & RAG (0.219) & RAG (0.312) & RAG (0.469) \\
\bottomrule
\end{tabular}%
}
\end{table}
...
coordinates {
    (0.7805,1) +- (0.0987,0.1131) % Exp8  grouped CI: [0.68, 0.89]
    (0.7442,2) +- (0.0775,0.0740) % Exp7  grouped CI: [0.67, 0.82]
    (0.8421,3) +- (0.0992,0.0954) % Exp6  grouped CI: [0.74, 0.94]
    (0.7742,4) +- (0.1075,0.1082) % Exp5  grouped CI: [0.67, 0.88]
    (0.7556,5) +- (0.0738,0.0777) % Exp4  grouped CI: [0.68, 0.83]
    (0.8000,6) +- (0.1125,0.1167) % Exp3  grouped CI: [0.69, 0.92]
    (0.6429,7) +- (0.2429,0.2033) % Exp2  grouped CI: [0.40, 0.85]
    (0.7059,8) +- (0.1459,0.1217) % Exp1  grouped CI: [0.56, 0.83]
};
...
\addplot[only marks, mark=*, green!60!black, thick,
    error bars/.cd, x dir=both, x explicit]
coordinates {
    (0.8125,9)  +- (0.1696,0.1161) % Exp8  grouped CI: [0.64, 0.93]
    (0.7586,10) +- (0.1872,0.1505) % Exp7  grouped CI: [0.57, 0.91]
    (0.8235,11) +- (0.1138,0.1051) % Exp6  grouped CI: [0.71, 0.93]
    (0.8125,12) +- (0.1696,0.1161) % Exp5  grouped CI: [0.64, 0.93]
    (0.7027,13) +- (0.1866,0.1360) % Exp4  grouped CI: [0.52, 0.84]
    (0.7742,14) +- (0.1588,0.1147) % Exp3  grouped CI: [0.62, 0.89]
    (0.8235,15) +- (0.1138,0.1051) % Exp2  grouped CI: [0.71, 0.93]
    (0.7273,16) +- (0.1440,0.1114) % Exp1  grouped CI: [0.58, 0.84]
};
...
\addplot[only marks, mark=*, red, thick,
    error bars/.cd, x dir=both, x explicit]
coordinates {
    (0.8000,17) +- (0.1000,0.0966) % Exp8  grouped CI: [0.70, 0.90]
    (0.8108,18) +- (0.0540,0.0610) % Exp7  grouped CI: [0.76, 0.87]
    (0.8485,19) +- (0.0743,0.0848) % Exp6  grouped CI: [0.77, 0.93]
    (0.8235,20) +- (0.1056,0.1051) % Exp5  grouped CI: [0.72, 0.93]
    (0.6667,21) +- (0.1667,0.1441) % Exp4  grouped CI: [0.50, 0.81]
    (0.7742,22) +- (0.1681,0.1401) % Exp3  grouped CI: [0.61, 0.91]
    (0.8235,23) +- (0.1056,0.1051) % Exp2  grouped CI: [0.72, 0.93]
    (0.6857,24) +- (0.2151,0.1587) % Exp1  grouped CI: [0.47, 0.84]
};
...
\addplot[only marks, mark=*, blue, thick,
    error bars/.cd, x dir=both, x explicit]
coordinates {
    (0.8571,25) +- (0.1238,0.1106) % Exp8  grouped CI: [0.73, 0.97]
    (0.8125,26) +- (0.0718,0.0841) % Exp7  grouped CI: [0.74, 0.90]
    (0.8571,27) +- (0.1142,0.1084) % Exp6  grouped CI: [0.74, 0.97]
    (0.8750,28) +- (0.0871,0.0905) % Exp5  grouped CI: [0.79, 0.97]
    (0.8205,29) +- (0.1753,0.1207) % Exp4  grouped CI: [0.65, 0.94]
    (0.7857,30) +- (0.1493,0.1374) % Exp3  grouped CI: [0.64, 0.92]
    (0.8000,31) +- (0.2400,0.1600) % Exp2  grouped CI: [0.56, 0.96]
    (0.7368,32) +- (0.1812,0.1521) % Exp1  grouped CI: [0.56, 0.89]
};
...
\caption{Forest plot of F1-Scores with grouped (cluster) bootstrap 95\% confidence intervals ($k=10{,}000$, resampling the nine meta-rule clusters) for all model-configuration pairs ($N=32$ artifacts). Error bars represent the CI range. No exact McNemar comparison survives Holm--Bonferroni correction at $\alpha=0.05$ ($m=28$). Confidence intervals overlap substantially across top configurations. The majority-class floor is F1=0.6939. Narrower intervals indicate more stable performance estimates.}
\label{fig:forest_plot}
\end{figure*}

\begin{table*}[htbp]
\centering
\caption{Complete experimental results across all evaluated configurations ($N=32$ artifacts): F1-Score, grouped (cluster) bootstrap 95\% confidence intervals ($k=10{,}000$, resampling the nine meta-rule clusters), Matthews Correlation Coefficient (MCC), balanced accuracy (BAcc), per-meta-rule macro-F1 (mF1), raw $p$-values of the exact cluster-level paired permutation test (primary; respects clustering) and of the exact McNemar test (secondary; treats artifact pairs as independent) against each model's baseline, and LOO-F1 (leave-one-out estimate of $\alpha$-selection optimism) for steering configurations. \textbf{Every Holm--Bonferroni adjusted $p$-value in both test families ($m=28$ each) equals 1.000, and no raw $p$-value reaches 0.05}; results should be interpreted as exploratory. Majority-class floor: F1=0.6939.}
\label{tab:full_results}

\resizebox{\textwidth}{!}{
\begin{tabular}{llcccccccc}
\hline
Model & Experiment & F1-Score & 95\% CI (grouped) & MCC & BAcc & mF1 & Perm.\ $p$ (raw) & McNemar $p$ (raw) & LOO-F1 \\
\hline

\multirow{8}{*}{Gemma-2-9B-it}
& Exp 1: Baseline         & 0.7368 & [0.56, 0.89] & 0.375 & 0.678 & 0.729 & --     & -- & -- \\
& Exp 2: Zero-Shot        & 0.8000 & [0.56, 0.96] & 0.649 & 0.820 & 0.719 & 0.4062 & 0.2891 & -- \\
& Exp 3: RAG              & 0.7857 & [0.64, 0.92] & 0.680 & 0.824 & 0.737 & 0.4062 & 0.3438 & -- \\
& Exp 4: Steering         & 0.8205 & [0.65, 0.94] & 0.583 & 0.771 & 0.791 & 1.0000 & 0.3750 & 0.7368 \\
& Exp 5: ZS + RAG         & \textbf{0.8750} & [0.79, 0.97] & 0.757 & 0.878 & 0.881 & 0.1250 & 0.0703 & -- \\
& Exp 6: ZS + Steering    & 0.8571 & [0.74, 0.97] & 0.686 & 0.841 & 0.869 & 0.1250 & 0.0625 & 0.8485 \\
& Exp 7: RAG + Steer      & 0.8125 & [0.74, 0.90] & 0.631 & 0.816 & 0.822 & 0.3750 & 0.2891 & 0.8889 \\
& Exp 8: ZS + RAG + Steer & 0.8571 & [0.73, 0.97] & 0.686 & 0.841 & 0.863 & 0.1250 & 0.0625 & 0.8235 \\

\hline

\multirow{8}{*}{Mistral-7B-Instruct-v0.2}
& Exp 1: Baseline         & 0.6857 & [0.47, 0.84] & 0.308 & 0.653 & 0.589 & --     & -- & -- \\
& Exp 2: Zero-Shot        & 0.8235 & [0.72, 0.93] & 0.624 & 0.812 & 0.841 & 0.1250 & 0.1250 & -- \\
& Exp 3: RAG              & 0.7742 & [0.61, 0.91] & 0.576 & 0.786 & 0.752 & 0.3594 & 0.2891 & -- \\
& Exp 4: Steering         & 0.6667 & [0.50, 0.81] & 0.177 & 0.582 & 0.667 & 1.0000 & 0.7539 & 0.6512 \\
& Exp 5: ZS + RAG         & 0.8235 & [0.72, 0.93] & 0.624 & 0.812 & 0.841 & 0.1250 & 0.1250 & -- \\
& Exp 6: ZS + Steering    & \textbf{0.8485} & [0.77, 0.93] & 0.689 & 0.845 & 0.859 & 0.1094 & 0.0703 & 0.8235 \\
& Exp 7: RAG + Steer      & 0.8108 & [0.76, 0.87] & 0.566 & 0.774 & 0.815 & 0.3125 & 0.2891 & 0.7568 \\
& Exp 8: ZS + RAG + Steer & 0.8000 & [0.70, 0.90] & 0.560 & 0.778 & 0.804 & 0.2188 & 0.2188 & 0.8235 \\

\hline

\multirow{8}{*}{Qwen2.5-7B-Instruct}
& Exp 1: Baseline         & 0.7273 & [0.58, 0.84] & 0.438 & 0.720 & 0.696 & --     & -- & -- \\
& Exp 2: Zero-Shot        & 0.8235 & [0.71, 0.93] & 0.624 & 0.812 & 0.832 & 0.3750 & 0.3750 & -- \\
& Exp 3: RAG              & 0.7742 & [0.62, 0.89] & 0.576 & 0.786 & 0.748 & 0.5000 & 0.5000 & -- \\
& Exp 4: Steering         & 0.7027 & [0.52, 0.84] & 0.307 & 0.649 & 0.684 & 0.6250 & 0.6250 & 0.6000 \\
& Exp 5: ZS + RAG         & 0.8125 & [0.64, 0.93] & 0.631 & 0.816 & 0.777 & 0.2500 & 0.2500 & -- \\
& Exp 6: ZS + Steering    & \textbf{0.8235} & [0.71, 0.93] & 0.624 & 0.812 & 0.832 & 0.3750 & 0.3750 & 0.8235 \\
& Exp 7: RAG + Steer      & 0.7586 & [0.57, 0.91] & 0.598 & 0.790 & 0.737 & 0.5000 & 0.6875 & 0.5556 \\
& Exp 8: ZS + RAG + Steer & 0.8125 & [0.64, 0.93] & 0.631 & 0.816 & 0.777 & 0.2500 & 0.2500 & 0.8125 \\

\hline

\multirow{8}{*}{Phi-3.5-mini}
& Exp 1: Baseline         & 0.7059 & [0.56, 0.83] & 0.372 & 0.686 & 0.678 & --     & -- & -- \\
& Exp 2: Zero-Shot        & 0.6429 & [0.40, 0.85] & 0.416 & 0.698 & 0.626 & 1.0000 & 1.0000 & -- \\
& Exp 3: RAG              & 0.8000 & [0.69, 0.92] & 0.649 & 0.820 & 0.826 & 0.3750 & 0.2188 & -- \\
& Exp 4: Steering         & 0.7556 & [0.68, 0.83] & 0.402 & 0.633 & 0.770 & 1.0000 & 1.0000 & 0.7907 \\
& Exp 5: ZS + RAG         & 0.7742 & [0.67, 0.88] & 0.576 & 0.786 & 0.789 & 0.5000 & 0.4531 & -- \\
& Exp 6: ZS + Steering    & \textbf{0.8421} & [0.74, 0.94] & 0.639 & 0.804 & 0.856 & 0.3125 & 0.3438 & 0.7429 \\
& Exp 7: RAG + Steer      & 0.7442 & [0.67, 0.82] & 0.351 & 0.637 & 0.751 & 1.0000 & 1.0000 & 0.7429 \\
& Exp 8: ZS + RAG + Steer & 0.7805 & [0.68, 0.89] & 0.470 & 0.704 & 0.789 & 1.0000 & 1.0000 & 0.7647 \\

\hline
\multicolumn{10}{l}{\footnotesize CIs are grouped-bootstrap percentile intervals resampling the nine meta-rule clusters with replacement; artifact-level i.i.d.\ intervals would understate dependence.} \\
\multicolumn{10}{l}{\footnotesize Perm.\ $p$: exact cluster-level paired sign-flip permutation test ($2^9=512$ patterns) on per-cluster correctness differences vs.\ Exp~1 (primary test).} \\
\multicolumn{10}{l}{\footnotesize McNemar $p$: exact two-sided binomial test on discordant correctness pairs vs.\ Exp~1 (secondary; assumes independent pairs). Holm-adjusted $p=1.000$ for all tests in both families.} \\
\multicolumn{10}{l}{\footnotesize Regex baseline (for reference): F1=0.6471 [0.59, 0.71], MCC=0.247, BAcc=0.624, mF1=0.667. TF-IDF baselines (leave-one-group-out): F1=0.63--0.70 (§\ref{sec:results}).} \\
\multicolumn{10}{l}{\footnotesize RAG configurations (Exp~3, 5, 7, 8) inject genuine normative context from \texttt{Metareglas extraidas.docx} (14 chunks, 3,864 chars).} \\
\multicolumn{10}{l}{\footnotesize LOO-F1: leave-one-out estimate for steering configurations (Exp~4, 6, 7, 8), internal to the benchmark (\emph{not} external validation); ``--'' = not applicable.} \\
\multicolumn{10}{l}{\footnotesize All steering experiments (Exp~4, 6, 7, 8) use Spanish-language anchor prompts (\texttt{lang=es}). Mean absolute LOO optimism $\approx 0.05$ (range $-0.20$ to $+0.09$); Qwen2.5 Exp~7 ($-0.20$): interpret with caution.} \\
\end{tabular}
}
\end{table*}
...
The results reveal complementary rather than competing effects between contextual grounding and activation-steered intervention. The majority-class floor on the $N=32$ dataset is F1=0.6939; baseline experiments cluster near this floor (F1 range: 0.686--0.737), while most multi-technique configurations substantially exceed it. The RAG pipeline (Experiments~3, 5, 7, and~8) injected genuine normative context from the 14-chunk ISO/IEC~29110 meta-rule corpus into every query, producing consistent positive $\Delta$F1 values across all four models (Fig.~\ref{fig:delta_f1}). The grounding effect is most pronounced in combinatorial configurations: Gemma-2 reaches its peak point estimate at Exp~5 ZS+RAG (F1=0.875). \textbf{RAG is the only single intervention with a uniformly positive direction of effect}: Exp~3 improves the baseline point estimate for all four models (McNemar discordant pairs favour RAG in every case: $c>b$ for Gemma-2 7:3, Mistral 6:2, Qwen2.5 2:0, Phi-3.5 5:1), although no individual comparison survives Holm--Bonferroni correction and none reaches even uncorrected significance.

Activation steering modified the model's internal activation state, directly shifting the internal classification boundary. Its effect, however, is conditional rather than universal. In isolation (Exp~4), steering \emph{decreases} F1 for Mistral (0.686 to 0.667) and Qwen2.5 (0.727 to 0.703), and the cost-aware metrics are less favourable still: Exp~4 lowers MCC relative to baseline for Mistral (0.177 vs.\ 0.308) and Qwen2.5 (0.307 vs.\ 0.438), and lowers balanced accuracy for Phi-3.5 (0.633 vs.\ 0.686) because its recall gain is bought with false-compliance declarations. Only in combination with structured prompting (Exp~6, ZS+Steering) does steering improve or match the corresponding prompt-only configuration for all four architectures, with Exp~6 the best steering configuration for Mistral, Qwen2.5, and Phi-3.5. The LOO procedure estimates the in-sample $\alpha$-selection optimism at mean $|$optimism$| \approx 0.05$ across all 16 steering-configuration/model pairs (range: $-0.20$ to $+0.09$); Qwen2.5 Exp~7 shows the largest negative optimism ($-0.20$) and should be interpreted with caution. This LOO estimate is internal to the benchmark and does not establish out-of-distribution generalization. Phi-3.5 shows a complex pattern: RAG alone gives its best non-steering result (Exp~3, F1=0.800), but ZS+Steering (Exp~6) achieves F1=0.842 with Spanish anchors, its best overall configuration.

\textbf{Steering gains are architecture- and context-dependent, not universally positive.} The ``Steer'' column of Fig.~\ref{fig:delta_f1} shows the \emph{negative} steering-only deltas for Mistral ($-0.019$) and Qwen2.5 ($-0.025$) with Spanish anchors, in contrast to the positive gains under ZS+Steering (Exp~6). This divergence indicates that the compliance direction extracted from the anchor prompts is useful only when the model's decoding is already oriented toward the auditing task by a structured prompt---steering alone is insufficient, and can be counterproductive. No individual steering comparison survives Holm--Bonferroni correction, and the magnitude of effects varies substantially, from $-0.019$ to $+0.163$ for ZS+Steering across models. All steering-related claims should be read as exploratory; any claim that steering is the ``most consistently beneficial'' single intervention is not supported by these data---that description belongs to RAG.
...
The proposed framework addresses two practical requirements for industrial deployment. First, the distributional calibration methodology enables compliance evaluation research without requiring access to proprietary repositories, removing a barrier that has historically limited empirical progress in this domain. Second, the use of open-weights models with local inference ensures that no corporate data leaves the organization's infrastructure, satisfying air-gapped deployment requirements.

The activation steering approach is particularly relevant for industrial settings because it does not require model fine-tuning or access to large labeled datasets. The steering vector can be extracted from two anchor prompts and applied at inference time. The LOO procedure on the $N=32$ dataset estimates the $\alpha$-selection optimism as small on average (mean $\approx 0.05$); this is an internal-consistency check on the benchmark, not evidence of generalization to other repositories or standards.
...
From the perspective of the framework's stated goal---reducing unsupported or inconsistent LLM classifications---steering addresses the problem at the mechanistic level by shifting the model's internal decision boundary toward normative compliance. The results show this shift increases recall in every architecture (Exp~4 reduces false negatives relative to baseline for all four models), but at the cost of additional false-compliance declarations: for Mistral, Qwen2.5, and Phi-3.5 the steering-only MCC or balanced accuracy is \emph{below} baseline. In an auditing context where falsely declaring compliance is the costlier error, steering should therefore be deployed only together with structured prompting (Exp~6), where the error profile rebalances.
...
On the original 20 artifacts, the Regex baseline achieves perfect F1=1.0000, which serves as a \emph{label-validity check} rather than a competitive comparison. The 12 adversarial artifacts expose the limits of this result: on the full $N=32$ set, the Regex drops to F1=0.6471 (grouped 95\,\% CI [0.59, 0.71]), indistinguishable from the majority-class floor of F1=0.6939, and the TF-IDF lexical classifiers reach only F1=0.63--0.70 under leave-one-group-out cross-validation. The benchmark is therefore no longer solvable by any of the evaluated surface-cue baselines.
...
Internal & \checkmark &
Leave-one-out (LOO) $\alpha$-selection procedure applied over $N=32$ (§IV-D, Table~\ref{tab:full_results}). Mean absolute optimism $\approx$0.05 (range $-0.20$ to $+0.09$). LOO is internal to the benchmark and does not cover layer, language, or model selection. \\
...
Conclusion & $\sim$ &
Exact McNemar tests on paired correctness replace the earlier Wilcoxon analysis; grouped bootstrap respects the mutation-cluster design. LOO bounds $\alpha$-selection optimism within the benchmark. Exploratory framing is maintained throughout. Statistical floor stated explicitly (majority-class F1=0.694). Scaling to $N\approx100$--200 is the top future-work priority. \\
...
Terminology & \checkmark &
Terminology revised to ``distributionally calibrated under selected metrics'' consistently throughout. ``Partial structural isomorphism'' appears only as a historical term with an explicit caveat (\S III-B); the calibration evidence does not establish comparable governance topology. \\
...
Construct & $\sim$ &
One author independently re-evaluated all $N=32$ artifacts; Cohen's $\kappa=0.813$ against mutation-protocol labels (§VII-A). This is a single, non-blinded, author-internal check, \emph{not} independent validation: the 3 disagreements fall on QUALITY, GOVERNANCE, and TRACEABILITY---the latter two central to the paper's semantic-performance claims. Validation by multiple blinded, independent ISO/IEC~29110 auditors with adjudication remains open and is required before the ground truth can be considered established. \\
...
\textbf{Adversarial artifact validation.} To address the construct-validity concern that the original 20-artifact benchmark is regex-solvable, 12 adversarial artifacts (6 Type-B negatives, 6 Type-C positives) were added. On the extended $N=32$ dataset, the Regex baseline drops from F1=1.000 to F1=0.6471 (grouped 95\,\% CI [0.59, 0.71]), comparable to the majority-class floor of F1=0.6939. TF-IDF lexical classifiers under leave-one-group-out cross-validation reach only F1=0.63--0.70. LLM-based configurations maintain F1=0.75--0.875, suggesting contextual discrimination beyond the evaluated regex and lexical baselines---though formatting/structural cues not covered by these baselines cannot be excluded without perturbation tests (future work). This substantially mitigates the original regex-solvability concern.

\textbf{Circularity between the RAG corpus and the benchmark labels.} The RAG document (\texttt{Metareglas extraidas.docx}), the operational meta-rules, the detection predicates of Table~\ref{tab:metarules}, the mutation protocol, and the ground-truth labels all originate from the same research design. Strong RAG performance therefore demonstrates, in the first instance, agreement with the authors' \emph{operationalization} of ISO/IEC~29110---the retrieved chunks describe the very rules from which the labels were derived---rather than compliance competence against the standard at large. Three distinct claims must be kept separate: (i) performance against the author-defined benchmark rules, which is what this paper measures; (ii) performance against independently interpreted ISO/IEC~29110 requirements, which would require normative material and labels produced by parties outside the research team; and (iii) process-level organizational compliance assessment, which artifact-level binary classification does not capture at all. The results reported here support only claim (i). Breaking this circularity---e.g., by retrieving from the official standard text rather than the author-extracted meta-rule digest, and by independent labeling---is a prerequisite for claims (ii) and (iii).
...
\label{eq:kappa}
\end{equation}
where $P_o = 29/32 = 0.906$ is the observed agreement and $P_e = 0.500$ is the agreement expected by chance (17 positives in the protocol, 16 in the human labels, both out of 32). Under the Landis--Koch scale, $\kappa = 0.813$ corresponds to \emph{almost perfect agreement}. The three disagreements occurred on meta-rules QUALITY (\texttt{SMP-CALIDAD-POS}: human labelled as violation), GOVERNANCE (\texttt{SMP-GOBERNANZA-NEG}: human labelled as compliant), and TRACEABILITY (\texttt{SMP-TRAZABILIDAD-POS}: human labelled as violation). All three fall in the semantically hard or intermediate zone identified in §V-F---precisely the rules where the human evaluator, like the LLMs, found the compliance criterion ambiguous.

The limits of this calibration must be stated plainly. The re-evaluation was performed by one of the authors, who designed the mutation protocol and cannot be considered blinded; a single rater also provides no estimate of inter-rater variability. The disagreements concentrate on GOVERNANCE and TRACEABILITY, which are central to the paper's claims about semantic auditing performance: if an independent auditor sided with the dissenting labels on those artifacts, the relative ranking of configurations on the semantically hard rules could change. The ground truth for Biblio-VSE should therefore be regarded as \emph{operationally defined by the mutation protocol and provisionally supported---not validated---by expert judgment}. Proper validation requires multiple independent, blinded ISO/IEC~29110 auditors labelling all 32 artifacts against the meta-rule criteria, with disagreements resolved by adjudication and inter-rater reliability reported (e.g., Fleiss' $\kappa$); this is planned future work and a precondition for any confirmatory study.
...
Regarding the activation steering vector: the compliance vector $v_{norma}^l$ is extracted exclusively from two contrasting behavioral anchor prompts ($p^+$, $p^-$) as described in Eq.~\ref{eq:steering_vector}, and does not reference or use any of the 32 Biblio-VSE evaluation artifacts during extraction. The vector extraction and the evaluation pipeline are therefore fully independent, and no form of circular evaluation or train-test contamination is present. This design decision was deliberate: the contrastive-prompt approach requires no labeled examples and is transferable to new compliance domains without access to labeled data.

A secondary internal validity concern is the joint selection of intervention layer $l$ and coefficient $\alpha$. Both were fixed without a traditional held-out validation split. Regarding $\alpha$: a leave-one-out (LOO) $\alpha$-selection procedure was conducted on the $N=32$ dataset. For each of the 16 steering-configuration/model pairs, the LOO-F1 was computed and compared to the in-sample F1. Results are tabulated in Table~\ref{tab:full_results}. Mean absolute optimism is approximately 0.05 (range: $-0.20$ to $+0.09$), with the largest negative optimism seen for Qwen2.5 Exp~7 ($-0.20$), where the RAG+Steering combination with Spanish anchors shows high LOO variance. Key results: Gemma-2 Exp~7 (LOO-F1=0.889 vs.\ in-sample 0.813, optimism=$+0.076$), Mistral Exp~6 (LOO-F1=0.824 vs.\ 0.848, optimism=$-0.025$), and Qwen2.5 Exp~7 (LOO-F1=0.556 vs.\ 0.759, optimism=$-0.203$). These values indicate that the in-sample F1 figures were not dramatically inflated by $\alpha$ selection; Qwen2.5 Exp~7 is the notable exception requiring caution. Two qualifications apply. First, LOO re-selects only $\alpha$: the intervention layer, anchor language, and the choice of best model/configuration were all fixed or selected on the same benchmark, so the LOO estimate is a lower bound on total selection optimism. Second, LOO operates entirely within Biblio-VSE; it says nothing about generalization to other repositories, languages, or standards, and the term ``out-of-sample'' is deliberately avoided. The caution about upper-bound estimates therefore remains appropriate.
...
\textbf{Scalability of the distributional calibration methodology.} The mapping $f$ was manually designed for one repository and one standard. Applying the same methodology at broader scale introduces practical constraints across three axes.
...
In each scenario, the core methodology---measure private corpus, generate distributionally similar synthetic artifacts, inject controlled violations---remains applicable in principle. The present contribution demonstrates feasibility for one standard, one organization, and one technology stack; scale-out toward automated calibration learning and multi-standard benchmarking is the most significant open challenge for this line of work.
...
The sample size ($N=32$) is larger than the initial 20-artifact dataset but still constrains the statistical power of the reported comparisons. The exact McNemar test depends on the discordant correctness pairs, which range from 2 to 13 across the 28 comparisons; a configuration must correct at least six baseline errors without introducing any new error before reaching even uncorrected significance at $\alpha=0.05$. Smaller accuracy differences remain undetectable at this scale. The grouped bootstrap intervals remain wide, indicating substantial uncertainty around point estimates; with only nine meta-rule clusters, the intervals themselves are approximate.

Evaluating 7 non-baseline configurations across 4 models yields 28 simultaneous comparisons. Holm--Bonferroni correction was applied (adjusted $p$-values reported in Table~\ref{tab:full_results}); no test survives correction at $\alpha=0.05$. Furthermore, the $m=28$ family does not cover the other selection dimensions exercised during the study---steering layer, $\alpha$ value, anchor language, and the post-hoc choice of best model and configuration among 32 cells---so every ``best'' cell reported here is additionally exposed to winner's-curse selection bias. The LOO $\alpha$-selection procedure quantifies optimism for one of these dimensions ($\alpha$) \emph{within} the benchmark; it is not external validation, and it does not address layer, language, or model selection. The results are therefore exploratory: the observed effect directions (in particular, RAG-alone improving every architecture) motivate confirmatory studies on larger datasets rather than constituting final evidence.
...
The exploratory empirical evaluation ($N=32$ artifacts, including 12 adversarial) yielded four findings, detailed in Sections~\ref{sec:results}--\ref{sec:threats}. In brief: (1) adversarial artifacts defeat all evaluated surface-cue baselines---Regex drops to F1=0.647 and TF-IDF classifiers reach only 0.63--0.70 while LLM-based pipelines maintain 0.75--0.875, suggesting contextual discrimination beyond those baselines (not proof of semantic reasoning); (2) RAG grounding is the only single intervention that improves the point estimate for every architecture, with Gemma-2 ZS+RAG (Exp~5, F1=0.875) as the highest point estimate; (3) activation steering is beneficial only in combination with structured prompting---ZS+Steering (Exp~6) improves or matches Zero-Shot for all four architectures, but steering in isolation (Exp~4) decreases F1 for Mistral and Qwen2.5 and worsens cost-aware metrics (MCC, balanced accuracy) for three of four models; the LOO procedure bounds the $\alpha$-selection optimism within the benchmark (mean $\approx 0.05$) but no steering gain survives Holm correction; and (4) the two semantically hard meta-rules (TESTING and GOVERNANCE source artifacts) resist all pipeline configurations, identifying the frontier for future mechanistic work. Pipeline configuration must be validated per model; a suboptimal choice can reduce F1 by more than 0.18 (Mistral Exp~4 vs.\ Exp~6).
...
In summary, this study demonstrates technical feasibility on one researcher-constructed synthetic benchmark: RAG and selected steering combinations produce promising, architecture-dependent point estimates, but no improvement is statistically confirmed. Independent expert labeling, stronger non-LLM baselines beyond those evaluated here, clustered confirmatory testing at larger scale, and evaluation on additional repositories are required before the framework can support claims about practical organizational auditing.

Several directions remain for future work. The immediate priority is scaling the evaluation to larger datasets with greater artifact diversity to establish external generalizability. Closely related is hardening the contextual-discrimination claim with stronger non-LLM baselines (sentence-embedding classifiers, static-analysis rules, independently constructed deterministic rules) and with paraphrase, cue-removal, renaming, and formatting-perturbation tests that isolate which cues the LLMs actually exploit. Validation on actual industrial repositories---using anonymization protocols or federated evaluation---would strengthen the practical relevance of the approach. Extending the framework to additional governance standards beyond ISO/IEC 29110 (e.g., CMMI, ISO 12207, ISO 9001) would demonstrate cross-standard applicability. Evaluating closed-source models (e.g., GPT-4, Claude) through prompt-only configurations would establish comparative baselines against open-weights architectures. Human expert agreement studies are needed to calibrate model performance against professional auditor judgments. Finally, automated isomorphism generation---where the structural mapping $f$ is learned rather than manually designed---represents a promising direction for scaling the benchmark construction process.
...
\begin{itemize}
    ...
    \item A canonical machine-readable matrix of all per-artifact predictions for the 32 model-configuration cells plus the regex baseline (\texttt{experimentos/predictions\_n32.json} and \texttt{.csv}), built and validated against the consolidated experiment logs by \texttt{experimentos/build\_predictions\_matrix.py}.
    \item The statistical analysis script and its output (\texttt{experimentos/statistical\_resampling.py}, \texttt{experimentos/statistical\_analysis\_n32.json}): cluster-level paired permutation tests, exact McNemar tests, grouped bootstrap CIs, MCC, balanced accuracy, per-rule macro-F1, and the asymmetric-cost analysis of Eq.~\ref{eq:cost}.
    \item The lexical baseline script and results (\texttt{experimentos/lexical\_baselines.py}, \texttt{experimentos/lexical\_baselines\_n32.json}): TF-IDF classifiers under leave-one-meta-rule-group-out cross-validation.
    \item Layer-sensitivity sweeps and LOO results (\texttt{experimentos/layer\_sensitivity\_*.json}, \texttt{experimentos/loo\_results\_*.json}).
\end{itemize}
Steering vectors are not distributed as serialized tensors; they are deterministic functions of the released anchor prompts and the public model checkpoints (two forward passes, Eq.~\ref{eq:steering_vector}), and are reproduced exactly by the released experiment scripts.
...
\begin{IEEEbiography}[{\includegraphics[width=1in,height=1.25in,clip,keepaspectratio]{a1.png}}]{Victor Terrón-Macias}
...
\begin{IEEEbiography}[{\includegraphics[width=1in,height=1.25in,clip,keepaspectratio]{a2.jpg}}]{Salvador Hinojosa}
...
\begin{IEEEbiography}[{\includegraphics[width=1in,height=1.25in,clip,keepaspectratio]{a3.jpg}}]{Miguel Gonzalez-Mendoza}
...
\begin{IEEEbiography}[{\includegraphics[width=1in,height=1.25in,clip,keepaspectratio]{al.jpg}}]{Jezreel Mej\'{i}a}
