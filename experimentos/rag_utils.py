...
CORPUS_FILES = {
    ...
    # Spanish edition of the same guide, used by the fully Spanish pipeline.
    "official_es": {"Parte 5-1-2 GuiadeGestioneIngenieria_PDS 2022.pdf"},
}
...
def cargar_documentos(corpus: str = "metarules") -> list[dict]:
    ...
    corpus="official" uses only the official ISO/IEC 29110 guide (English).
    corpus="official_es" uses only its Spanish edition.
    ...
    if corpus not in {"metarules", "official", "official_es", "all"}:
        raise ValueError("corpus must be one of: metarules, official, official_es, all")
