# EXPERIMENT 2 — ZERO-SHOT (properly applied)
# Detailed system instruction + structured prompt with explicit criteria per rule.
# No examples, no documents, no steering.
import json, os, sys
...
def main():
    print(f"\n{'#'*70}")
    print(f"  {NOMBRE}")
    print(f"  ISO/IEC 29110 system instruction + explicit criteria per meta-rule")
    print(f"{'#'*70}")
